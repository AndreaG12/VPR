...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if args.self_supervised_learning:
            ...
            imgFinal1 = torch.stack((img[0], img[1], img[2]))
            imgFinal2 = torch.stack((img[0], img[2], img[1]))
            imgFinal3 = torch.stack((img[1], img[0], img[2]))
            imgFinal4 = torch.stack((img[1], img[2], img[0]))
            imgFinal5 = torch.stack((img[2], img[0], img[1]))
            imgFinal6 = torch.stack((img[2], img[1], img[0]))
            
            #print(imgFinal.shape)
            Image.fromarray(imgFinal1.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('2.jpg')
            Image.fromarray(imgFinal2.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('3.jpg')
            Image.fromarray(imgFinal3.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('4.jpg')
            Image.fromarray(imgFinal4.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('5.jpg')
            Image.fromarray(imgFinal5.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('6.jpg')
            Image.fromarray(imgFinal6.cpu().numpy().transpose(1,2,0).astype(np.uint8)).save('7.jpg')
